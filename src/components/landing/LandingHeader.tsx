      ...
      <div className="container mx-auto flex items-center justify-between h-full px-8">
        <Link to="/" className="flex items-center gap-3 group">
          <motion.div 
            ...
            className="w-10 h-10 rounded-xl gradient-bg flex items-center justify-center shadow-lg shadow-primary/20 transition-all"
          >
            <span className="text-white font-black text-base tracking-tighter">IM</span>
          </motion.div>
          <span className="font-display text-2xl font-bold tracking-tighter bg-gradient-to-r from-foreground to-foreground/70 bg-clip-text text-transparent group-hover:to-primary transition-all duration-300">InfluMatch</span>
        </Link>
        ...
        <nav className="hidden lg:flex items-center gap-12">
          {['Marketplace', 'Technology', 'Network', 'Pricing'].map((item) => (
            <a 
              ...
              className="text-xs font-black uppercase tracking-[0.2em] text-muted-foreground hover:text-primary transition-colors duration-300 relative group"
            >
              {item}
